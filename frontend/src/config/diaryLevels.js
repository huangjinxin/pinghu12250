/**
 * 日记字数级别配置
 * 与后端 backend/src/config/diaryLevels.js 同步
 */
 ...
 * @returns {object} 级别信息
 */
export function getDiaryLevel(wordCount) {
 ...
 * @returns {object} 级别信息
 */
export function getDiaryLevelByNumber(level) {
 ...
 */
export function getLevelProgress(wordCount, level) {
  if (level.level === DIARY_LEVELS.length - 1) {
    // 已是最高级别
    return 100
  }
  const nextLevel = DIARY_LEVELS[level.level + 1]
  const range = nextLevel.minWords - level.minWords
  const progress = wordCount - level.minWords
 ...
 * @param {number} wordCount - 当前字数
 * @param {object} level - 当前级别
 * @returns {number|null} 还需字数，已满级返回null
 */
export function getWordsToNextLevel(wordCount, level) {
  if (level.level === DIARY_LEVELS.length - 1) {
    return null
  }
  const nextLevel = DIARY_LEVELS[level.level + 1]
  return Math.max(0, nextLevel.minWords - wordCount)
}
